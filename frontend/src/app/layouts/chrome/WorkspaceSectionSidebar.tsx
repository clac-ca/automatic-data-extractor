...
import { getWorkspaceSecondaryNavigation } from "../../workspaces/navigation";
...
export function WorkspaceSectionSidebar({
  ...
  const navigation = getWorkspaceSecondaryNavigation(workspaceId, section);
                ...
                <li key={item.id}>
                  ...
                  >
                    <span className="flex items-center gap-3">
                      ...
                      <span
                        className={clsx(
                          "flex min-w-0 flex-1 flex-col overflow-hidden text-left text-sm transition-[opacity,transform] duration-200 ease-out",
...
function LoadingState({ collapsed, message }: { readonly collapsed: boolean; readonly message: string }) {
  return (
    <div className="flex flex-col gap-2 px-2 py-2">
      ...
      <p className={clsx("text-center text-xs text-slate-400", collapsed && "sr-only")}>{message}</p>
    </div>
  );
}
