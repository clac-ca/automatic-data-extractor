...
import { getWorkspacePrimaryNavItems, getWorkspaceSecondaryNavItems } from "../../workspaces/navigation";
...
export function WorkspaceNavigationDrawer({
  ...
  const secondaryItems = getWorkspaceSecondaryNavItems(workspace.id, section);
      ...
      >
        ...
        <div className="flex flex-col gap-6 overflow-y-auto px-4 py-5">
          ...
          {secondaryItems.length > 0 ? (
            <NavigationGroup
              title={section.label}
              items={secondaryItems.map((item) => ({ ...item, description: item.badge }))}
