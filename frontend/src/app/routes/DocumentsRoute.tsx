...
export function DocumentsRoute() {
    ...
    openInspector({
      ...
      content: (
        <DocumentInspector
          ...
          onRetry={() => trackDocumentAction("retry", workspace.id, activeDocument.id)}
          onOpenRuns={() => trackDocumentAction("open_runs", workspace.id, activeDocument.id)}
        />
      ),
      onClose: cleanup,
    });
  ...
  if (!hasDocuments) {
    return (
      <EmptyAllState
        onUpload={() => trackDocumentAction("upload", workspace.id)}
        onConnectSource={() => trackDocumentAction("connect_source", workspace.id)}
      />
    );
  }

  if (!hasFilteredDocuments) {
    return <EmptyFilteredState onClearFilters={handleClearFilters} />;
  }
  ...
  return (
    <section className="space-y-6">
      <header className="space-y-4">
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button type="button" onClick={() => trackDocumentAction("upload", workspace.id)}>
              Upload
            </Button>
          </div>
        </div>
      ...
      {selectedCount > 0 ? (
        <BulkActionBar
          count={selectedCount}
          onRetry={() => trackDocumentAction("bulk_retry", workspace.id)}
          onArchive={() => trackDocumentAction("bulk_archive", workspace.id)}
          onDelete={() => trackDocumentAction("bulk_delete", workspace.id)}
        />
      ) : null}
    </section>
  );
              ...
              >
                ...
                <td className="px-4 py-3 text-right">
                  <DocumentActionsMenu
                    ...
                    onAction={(action) => trackDocumentAction(action, workspaceId, document.id)}
                  />
                </td>
              </tr>
            );
...
function DocumentActionsMenu({ documentId, onOpenDetails, onAction }: DocumentActionsMenuProps) {
  ...
  return (
    <div className="relative inline-flex">
      <button
        ref={anchorRef}
        type="button"
        className="inline-flex h-9 w-9 items-center justify-center rounded-lg border border-slate-200 text-slate-500 transition hover:border-brand-200 hover:text-brand-600"
        aria-haspopup="menu"
        aria-expanded={open}
...
function BulkActionBar({ count, onRetry, onArchive, onDelete }: BulkActionBarProps) {
  return (
...
function EmptyAllState({ onUpload, onConnectSource }: { readonly onUpload: () => void; readonly onConnectSource: () => void }) {
  return (
    <section className="flex flex-col items-center justify-center rounded-3xl border border-dashed border-slate-300 bg-white px-6 py-16 text-center">
      ...
      <div className="mt-6 flex flex-wrap items-center justify-center gap-3">
        ...
        <Button type="button" variant="secondary" onClick={onConnectSource}>
          Connect source
        </Button>
      </div>
