...
from ade_api.common.etag import canonicalize_etag
...
class ConfigurationsService:
    ...
    def __init__(self, *, session: Session, storage: ConfigStorage) -> None:
    ...
    def import_configuration_from_archive(
        self,
        *,
        workspace_id: UUID,
        display_name: str,
    ...
    def replace_configuration_from_archive(
        ...
        *,
        ...
        config_path = self._storage.ensure_config_path(workspace_id, configuration_id)
        current_fileset_hash = self._current_fileset_hash(config_path)
    ...
    def read_file(
        ...
        *,
        ...
        info = _read_file_info(file_path, rel_path, include_content)
        logger.info(
    ...
    def export_zip(
        ...
        *,
        ...
        config_path = self._storage.ensure_config_path(workspace_id, configuration_id)
        data = _build_zip_bytes(config_path)
    ...
    def _materialize_source(
        ...
        *,
        ...
        if isinstance(source, ConfigSourceTemplate):
            ...
            self._storage.materialize_from_template(
                workspace_id=workspace_id,
                configuration_id=configuration_id,
            )
            return
        ...
        if isinstance(source, ConfigSourceClone):
            ...
            self._storage.materialize_from_clone(
                workspace_id=workspace_id,
                source_configuration_id=source.configuration_id,
    ...
    def _archive_active(self, workspace_id: UUID, exclude: UUID) -> None:
        existing = self._repo.get_active(workspace_id)
...
def _is_allowed_directory(rel_path: PurePosixPath) -> bool:
    if rel_path == PurePosixPath(""):
        return True
    if any(part in CONFIG_EXCLUDED_NAMES for part in rel_path.parts):
        return False
    return True
...
def _ensure_allowed_file_path(root: Path, rel_path: PurePosixPath) -> Path:
    ...
    if rel_path.suffix in CONFIG_EXCLUDED_SUFFIXES or rel_path.name in CONFIG_IGNORED_FILENAMES:
        raise PathNotAllowedError(f"{rel_path} is excluded")
...
def _build_file_index(config_path: Path) -> dict:
    ...
    for dirpath, dirnames, filenames in os.walk(config_path):
        ...
        for name in list(dirnames):
            ...
            if not _is_allowed_directory(rel) or name in CONFIG_EXCLUDED_NAMES:
        ...
        for filename in filenames:
            ...
            if any(part in CONFIG_EXCLUDED_NAMES for part in rel.parts):
                continue
            if rel.suffix in CONFIG_EXCLUDED_SUFFIXES or rel.name in CONFIG_IGNORED_FILENAMES:
                continue
            _add_file(rel)
