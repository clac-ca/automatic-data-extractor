...
def _apply_filters(statement: Select[AuditEvent], filters: dict[str, Any]) -> Select[AuditEvent]:
    ...
    if occurred_after is not None:
        ...
        statement = statement.where(AuditEvent.occurred_at >= occurred_after_dt.isoformat())

    occurred_before = filters.get("occurred_before")
    if occurred_before is not None:
        occurred_before_dt = _normalise_datetime(occurred_before)
        statement = statement.where(AuditEvent.occurred_at <= occurred_before_dt.isoformat())
