...
async def test_rename_column_updates_manifest_and_file(
    async_client: AsyncClient, seed_identity: dict[str, object]
) -> None:
    """Renaming a column updates the manifest and underlying script."""
    ...
    create_response = await async_client.post(
        collection_url,
        headers=headers,
        json={"title": "Rename Config"},
    )
    create_response.raise_for_status()
    config_id = create_response.json()["config_id"]

    rename_response = await async_client.post(
        f"{workspace_base}/configs/{config_id}/rename",
        headers=headers,
        json={"from_key": "member_id", "to_key": "member_identifier"},
    )
    assert rename_response.status_code == 200, rename_response.text
    updated_manifest = rename_response.json()
    assert "member_identifier" in updated_manifest["columns"]["meta"]
    assert "member_id" not in updated_manifest["columns"]["meta"]
    assert "member_identifier" in updated_manifest["columns"]["order"]
    assert updated_manifest["columns"]["meta"]["member_identifier"]["script"] == (
        "columns/member_identifier.py"
    )
    ...
    assert "columns/member_identifier.py" in paths
    assert "columns/member_id.py" not in paths

    script_response = await async_client.get(
        f"{workspace_base}/configs/{config_id}/files/columns/member_identifier.py",
        headers=headers,
    )
    script_response.raise_for_status()
    assert "detect_" in script_response.text


async def test_rename_column_conflict_preserves_original_file(
    async_client: AsyncClient, seed_identity: dict[str, object]
) -> None:
    """Conflicting renames fail without mutating manifest or scripts."""
    ...
    create_response = await async_client.post(
        collection_url,
        headers=headers,
        json={"title": "Rename Conflict"},
    )
    create_response.raise_for_status()
    config_id = create_response.json()["config_id"]

    file_headers = {**headers, "Content-Type": "text/plain"}
    put_response = await async_client.put(
        f"{workspace_base}/configs/{config_id}/files/columns/member_uuid.py",
    ...
    rename_response = await async_client.post(
        f"{workspace_base}/configs/{config_id}/rename",
        headers=headers,
        json={"from_key": "member_id", "to_key": "member_uuid"},
    )
    ...
    manifest = manifest_response.json()
    assert "member_id" in manifest["columns"]["meta"]
    assert manifest["columns"]["meta"]["member_id"]["script"] == "columns/member_id.py"
    assert "member_uuid" not in manifest["columns"]["meta"]
    ...
    paths = {item["path"] for item in files_response.json()}
    assert "columns/member_id.py" in paths
    assert "columns/member_uuid.py" in paths
