...
from ade_db.models import ApplicationSetting, GroupMembership, SsoIdentity, SsoProviderStatus, User
...
async def test_callback_sso_sets_external_id_without_group_sync_side_effects(
    async_client: AsyncClient,
    session: Session,
    settings: Settings,
    monkeypatch: pytest.MonkeyPatch,
    ...
    idp_membership = session.execute(
        select(GroupMembership)
        .where(GroupMembership.user_id == user.id)
        .where(GroupMembership.membership_source == "idp")
        .limit(1)
    ).scalar_one_or_none()
    assert idp_membership is None
