...
from backend.app.shared.core.config import get_settings, reload_settings
from backend.app.shared.db.session import get_sessionmaker
from backend.app.features.auth.service import (
    SSO_STATE_COOKIE,
    AuthService,
    OIDCProviderMetadata,
)
from backend.app.features.users.repository import UsersRepository
...
async def test_sso_callback_rejects_state_mismatch(
    ...
    monkeypatch.setenv(
        "ADE_OIDC_REDIRECT_URL", "https://backend.app.example.com/auth/sso/callback"
    )
    monkeypatch.setenv(
