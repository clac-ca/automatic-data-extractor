...
import typer
from ade_api.commands import common, server
...
def test_run_dev_defaults_to_single_process_reload(monkeypatch):
    ...
    monkeypatch.setattr(server, "Settings", lambda: _stub_settings(api_processes=9))
    monkeypatch.setattr(server, "_prepare_env", lambda: {"PATH": "test"})
    monkeypatch.setattr(common, "uvicorn_path", lambda: "uvicorn")
    ...
    monkeypatch.setattr(common, "run", _fake_run)
    ...
    assert reload_dirs == list(server.DEV_RELOAD_DIRS)
    assert captured["cwd"] == common.REPO_ROOT
...
def test_run_dev_processes_flag_disables_reload(monkeypatch):
    ...
    monkeypatch.setattr(server, "Settings", lambda: _stub_settings(api_processes=1))
    monkeypatch.setattr(server, "_prepare_env", lambda: {"PATH": "test"})
    monkeypatch.setattr(common, "uvicorn_path", lambda: "uvicorn")
    monkeypatch.setattr(
        common,
