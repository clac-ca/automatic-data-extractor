...
Below is an **implementation sketch** (simplified but faithful). You can drop this into `backend/app/kernel/build/prepare.py` and wire it to your “activate config” path.

```python
# backend/app/kernel/build/prepare.py
...
```python
# backend/app/features/jobs/launcher.py (sketch)
