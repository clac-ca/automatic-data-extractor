...
\# after loop:  
if max_field is not None and max_score >= manifest\["engine"\]\["defaults"\].get("min_mapping_confidence", 0.0):  
...
- When `allow_net` is true (or offline sources are configured), run `pip install --target <temp_dir> -r requirements.txt` to install packages into an isolated directory.
...
- If `allow_net` is false and a requirements file exists, ADE should fail fast with a clear message (preferred) or skip installation with an explicit warning, avoiding a later import error.
...
- Update documentation to explain how to add `requirements.txt`, remind authors to pin versions, and reiterate the `allow_net` requirement or the vendoring alternative.
...
Document that if `allow_net` is false (no internet) or if teams prefer offline installs, they can bundle dependencies by:
...
- **ade/rule_executor.py** (pipeline orchestration):
  - **Pass 2 (mapping):** apply `min_mapping_confidence`, leaving ties or low scores unmapped and capturing sorted scores for artifact output.
...
- **ade/requirements_installer.py** (new, or part of job initialization):
  ...
  - Respect `allow_net`; fail fast or skip installation if networking is disallowed.
...
- It sets a `min_mapping_confidence` of 0.5, mapping columns only when at least 50% confident.
...
For this example, maybe no external requirements are needed, as we can rely on Python stdlib for regex and date parsing. If we wanted to use python-dateutil for date parsing, we'd add it here and set allow_net: true or vendor it.
...
- **Pass 2:**
  ...
  - "Notes" stays unmapped; with a `min_mapping_confidence` of 0.5, it remains as `raw_Notes`.
- **Pass 3:**
...
- **Negative tests:** provide malformed manifests and confirm immediate, descriptive errors. Combine `allow_net: false` with `requirements.txt` to ensure we fail fast with guidance to enable networking or vendor packages.
...
- **Scoring and Mapping:** Introduce `min_mapping_confidence` (default 0) with an example of avoiding dubious mappings by raising the threshold.
...
- **Dependencies in Configs:** Explain `requirements.txt` support, when to set `allow_net: true`, and how to vendor libraries or pin versions. Note that installs are per-job and cleaned up.
