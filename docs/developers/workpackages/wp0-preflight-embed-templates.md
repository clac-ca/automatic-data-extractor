...
## Scope
* Relocate `templates/config-packages/*` into `apps/ade-api/src/ade_api/templates/config_packages/*`.
* Ensure `apps/ade-api/pyproject.toml` (and any relevant build/publish target) includes `templates/config_packages/**` as package data.
* Refresh developer docs to reference the new path and remove mention of template databases or environment variables.

## Deliverables
1. Files physically present under `apps/ade-api/src/ade_api/templates/config_packages/<template_id>/`.
