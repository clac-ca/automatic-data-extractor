# Work Packages — Config Authoring, Environments, and Runs
...
* [02-build-venv.md](../02-build-venv.md) — how environments create isolated virtual environments and how runs consume them.
...
| [WP5](./wp5-build-system.md) | Environment System: DB & Provisioning | Environment records, venv creation, dedupe, healing, and pruning. |
| [WP6](./wp6-runs-integration.md) | Runs Integration | Run runs in ensured environments and store environment selectors on run rows. |
...
The intent is to keep each package independently testable while funneling all creation/edit/build flows through a single, auditable path. With templates now coming from the engine CLI, backend template directories are no longer part of the flow.
