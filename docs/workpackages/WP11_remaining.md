...
> **Before touching this work package, read `apps/web/AGENTS.md` in full.** It explains the routerless mental model, folder contracts, and navigation expectations we must follow.
...
- [x] **Config Builder adapters & boundaries.** Either move `apps/web/src/screens/Workspace/api/adapters/*` into `screens/Workspace/sections/ConfigBuilder/adapters/` or re-export them behind a stable alias (`@screens/Workspace/sections/ConfigBuilder/adapters`). Update every consumer (e.g., `components/Workbench.tsx`, `hooks/useEditorState.ts`, `state/editorStore.ts`) so no file reaches back into `../api/adapters`.
...
- [x] **Curated schema exports.** Established `apps/web/src/schema/` with re-exports sourced from `@generated-types/openapi` (SessionEnvelope, WorkspaceProfile, UserSummary, etc.) so screens import via `@schema` only.
...
- [x] **Screen switch coverage.** Add unit tests that exercise `normalizePathname` and the screen routing matrix in `apps/web/src/app/App.tsx`. Include assertions for `/`, `/login`, `/auth/callback`, `/setup`, `/workspaces`, `/workspaces/:id`, `/workspaces/:id/config-builder`, `/workspaces/:id/config-builder/:configId/editor`.
...
- [x] **Testing utilities.** Update `apps/web/src/test/test-utils.tsx` so every render helper wraps components in `<NavProvider>`. Adjust existing tests to remove bespoke wrappers.
- [x] **History regression tests.** Extend `apps/web/src/app/nav/__tests__/urlState.test.tsx` (or add a sibling) to push multiple history entries, dispatch `popstate`, and assert `useLocation()` reflects pathname/search/hash changes.
- [x] **FastAPI SPA fallback.** Add a regression test around `apps/api/app/web/spa.py` (or manual verification notes) confirming `/workspaces/<id>/config-builder` refreshes return `index.html` while `/api/*` stays server-handled.
...
- [x] **Import audit.** Run `rg "react-router" apps/web/src` and ensure zero matches. Also check for old relative paths referencing `app/routes` or `../../routes`. Update offenders to use the new aliases (`@app/nav/*`, `@screens/*`).
...
- [x] **README cleanup.** Rewrite `apps/web/README.md` so it no longer references Remix/React Router templates. Highlight the history-based nav, screen-first folder layout, and path aliases.
...
- [ ] **Full CI.** `npm run ci` must be green. Resolve or document the existing Ruff backend lint errors so the pipeline is actionable.
