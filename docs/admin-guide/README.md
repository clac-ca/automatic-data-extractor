...
- Development uses `ade dev` to run the API, Vite dev server, and worker together (migrations run first). Use `--no-worker`, `--api-only`, `--web-only`, or `--worker-only` when you want fewer services. For production-style runs, build first (`ade build`) and then use `ade start` for the single-container flow (API + worker). Use `ade api start` / `ade worker start` for split containers.
