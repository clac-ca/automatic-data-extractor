...
- Development uses `ade-api dev` plus `ade-worker start` and `npm run dev --prefix apps/ade-web` in separate terminals. Run `ade-api migrate` before starting services. For production-style runs, build first (`npm run build --prefix apps/ade-web`) and then run split containers (`ade-api start` + `ade-worker start` + nginx via `/usr/local/bin/ade-web-entrypoint`).
...
- The API and worker expect the schema to be migrated before startup. Run `ade-api migrate` explicitly before starting services (see the [admin getting started guide](getting_started.md#manual-migrations-and-recovery)).
