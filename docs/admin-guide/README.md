...
- Development uses `ade dev` to run the API, Vite dev server, and worker together (migrations run first). Use `--no-worker`, `--api-only`, `--web-only`, or `--worker-only` when you want fewer services. Use `ade start` for the single-container flow (API + worker). Use `ade api start` / `ade worker start` for split containers. Add `--no-web` if you serve the frontend separately.
...
- Persistent state lives under the `data/` directory by default. Workspaces store document blobs beneath `data/workspaces/<workspace_id>/files/<file_id>` when using the filesystem backend. Override the root with `ADE_DATA_DIR` when relocating storage.
