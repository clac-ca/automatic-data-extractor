...
- Persistent state lives under the `data/` directory by default. SQLite databases sit under `data/db/`, and each workspace stores documents beneath `data/workspaces/<workspace_id>/documents/`. Override the root with `ADE_DATA_DIR` (and/or `ADE_DATABASE_URL`) when relocating storage.
...
- `ADE_DATABASE_URL` defaults to SQLite (`sqlite:///./data/db/ade.sqlite`) under `ADE_DATA_DIR`. Point it at Azure SQL with an `mssql+pyodbc` URL when deploying.
