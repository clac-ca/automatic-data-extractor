...
sessions and skip CSRF enforcement because they are intended for non-browser clients.【F:apps/api/app/shared/dependency.py†L94-L145】
3. **API keys** – created through the admin UI or API. The raw token is shown once and clients must provide it via the
`X-API-Key` header. API keys reuse the principal model so they participate in the same RBAC assignments.【F:apps/api/app/shared/dependency.py†L147-L150】【F:apps/api/app/features/auth/service.py†L52-L103】
...
exclusively.【F:apps/api/app/shared/dependency.py†L94-L150】
...
`AuthService.enforce_csrf`, which compares the hashed token in the JWT with the submitted value.【F:apps/api/app/shared/dependency.py†L170-L187】
...
requirements and the responses are consistent.【F:apps/api/app/shared/dependency.py†L154-L307】

* `require_authenticated` guarantees a `User` is attached to the request. All routers mount this dependency once so individual
handlers can assume an authenticated context.【F:apps/api/app/shared/dependency.py†L154-L167】
...
when access is denied.【F:apps/api/app/shared/dependency.py†L189-L240】
* `require_permissions_catalog_access` protects the permissions catalogue endpoints and handles both global and workspace
scopes. It reuses the same forbidden payload shape and performs scope validation for workspace requests.【F:apps/api/app/shared/dependency.py†L242-L305】
...
and client UX.【F:apps/api/app/shared/dependency.py†L189-L305】【F:apps/api/app/features/roles/authorization.py†L1-L123】
