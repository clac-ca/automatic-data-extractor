...
1. List the global role catalog and locate the `global-administrator` role to
   ...
   slug and permission set.【F:apps/api/app/features/roles/router.py†L76-L173】
2. Convert the target user into a principal. Supplying `user_id` in the role
   ...
   does not already exist.【F:apps/api/app/features/roles/router.py†L538-L613】
3. Assign the global administrator role by posting to
   ...
   idempotent, so repeated calls simply return the existing binding.【F:apps/api/app/features/roles/router.py†L538-L613】
4. Verify the grant through `GET /api/v1/me/permissions` while authenticated as
   ...
   confirm `Roles.ReadWrite.All` and the other administrator keys are present.【F:apps/api/app/features/roles/router.py†L200-L291】
...
1. Review the permission registry to decide which workspace capabilities the
   ...
   `Workspace.Jobs.ReadWrite`.【F:apps/api/app/features/roles/registry.py†L1-L132】
2. Create a workspace-scoped role by posting to
   ...
   persists the definition for future reuse.【F:apps/api/app/features/roles/router.py†L180-L398】
3. Assign the editor role to a user by calling
   ...
   a stable shape for auditing.【F:apps/api/app/features/roles/router.py†L651-L760】
4. Confirm the assignment via the workspace role-assignment list or by querying
   ...
   validate the editor capabilities.【F:apps/api/app/features/roles/router.py†L651-L760】【F:apps/api/app/features/roles/router.py†L200-L291】
