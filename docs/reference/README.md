# Reference

Stable facts for ADE commands, contracts, defaults, and system behavior.

Back to [Docs Home](../README.md).

## Core References

- [CLI Reference](cli-reference.md)
- [Environment Variables](environment-variables.md)
- [Defaults Matrix](defaults-matrix.md)
- [Runtime Lifecycle](runtime-lifecycle.md)
- [Release Process](release-process.md)
- [Azure Infra Bicep Reference](azure-bicep-parameters-and-naming.md)

## Architecture and Contracts

- [Backend Service Module Contract](backend-service-module-contract.md)
- [Auth Architecture](auth-architecture.md)
- [Access Reference](access/README.md)
- [API Capability Map](api-capability-map.md)

## Subsections

- [API Reference](api/README.md)
- [Service Reference](services/README.md)
