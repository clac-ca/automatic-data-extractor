...
## Job
An extraction request that consumes an input document and configuration revision. Job submission and monitoring routes are defined in [`apps/ade-api/src/ade_api/features/jobs`](../../apps/ade-api/src/ade_api/features/jobs). Execution happens inline through the pluggable processor contract exposed by [`apps/ade-api/src/ade_api/features/jobs/processor.py`](../../apps/ade-api/src/ade_api/features/jobs/processor.py).

## Configuration
Workspace-scoped packages containing draft files, manifests, and published snapshots. Configuration APIs live in [`apps/ade-api/src/ade_api/features/configs`](../../apps/ade-api/src/ade_api/features/configs), while jobs reference a concrete `config_version_id`.
