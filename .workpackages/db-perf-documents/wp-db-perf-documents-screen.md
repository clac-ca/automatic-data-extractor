...
- `logs/uploads/ade-documents-screen-prod-slow.log` — Run against the slow prod SQL Server (auth disabled for convenience). Startup: MSSQL migrations + registry sync ~1.9s. `/api/v1/bootstrap` 3.63s, dominated by dev identity creation + global admin assignment on the remote DB; permissions/roles then membership + safe-mode were fast. `/api/v1/workspaces/{id}/documents` 1.53s with cached registry/slug/assignment; document query itself ~140ms, remainder in dev identity path on the slow DB. Shows impact of network/DB latency even with caches warm.
