...
- **Completed Work:**
  ...
  - Phase 4 (in progress) – Restored document downloads with a streaming `/documents/{document_id}/download` route, service helpers that resolve stored files, timeline events for `document.downloaded`, and integration tests covering success, permission, and missing-file flows.
  - Phase 4 (in progress) – Implemented document soft deletion with a service helper that marks records deleted, removes stored files, emits `document.deleted` timeline events, exposes a guarded `DELETE /documents/{document_id}` route, and integration coverage for success, permission, missing-document, and idempotent retry paths (capturing a follow-up to design hard purge/TTL handling once retention requirements are defined).
  - Phase 4 (in progress) – Added metadata editing with a `DocumentMetadataUpdateRequest`, service helper, and guarded `PATCH /documents/{document_id}` route that merges changes, removes cleared keys, publishes `document.metadata.updated` or custom events, and exercises integration coverage for hub payloads, timeline diffs, permissions, and missing documents (follow-up: add event metadata overrides and stale-write guards once client contracts settle).
...
- Continue Phase 4 by adding optimistic concurrency controls to document metadata updates: require clients to supply the current `updated_at` (or an equivalent ETag) before applying changes, return HTTP 409 on stale edits, and expand tests to cover conflicting updates alongside success and permission paths.
