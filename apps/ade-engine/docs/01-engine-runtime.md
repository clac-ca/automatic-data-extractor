...
- **Sources**: `input_files` or `input_dir` (plus optional `input_sheets`).
...
# Convenience helper
result = run(input_files=[Path("input.xlsx")], output_dir=Path("output"), logs_dir=Path("logs"))
```
...
- `input_files: Sequence[Path] | None` **xor** `input_dir: Path | None` — source selection.
- `input_sheets: Sequence[str] | None` — optional sheet filter for XLSX.
- `output_dir: Path | None` — where normalized workbooks are written (default: `<input_dir>/output`).
- `logs_dir: Path | None` — where telemetry is written (default: `<input_dir>/logs`).
- `metadata: Mapping[str, Any] | None` — opaque telemetry tags.

Invariants: exactly one of `input_files` or `input_dir` must be provided; paths
are normalized to absolute paths early in `Engine.run`.
...
1. If `input_dir` is set, use it; otherwise, use the parent of the first `input_files` entry.
2. If `output_dir`/`logs_dir` are set, use them; otherwise, default to siblings of `input_dir` (`output/`, `logs/`).
3. Directories are created on demand before pipeline execution.
