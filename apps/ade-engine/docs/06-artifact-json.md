...
derives **run summaries** and uses **ADE events** (`events.ndjson`) to stream
a timeline.
...
- How ADE API is expected to consume it and relate it to telemetry and
  run summaries.
...
Artifact docs stick to these terms; ADE events and run summaries reuse the same
vocabulary.
...
- **Deliberately minimal**  
  ...
  **Per-row detail** and “live timeline” are expressed via ADE events in
...
* Config scripts: sparingly, for durable annotations (use ADE events for
  noisy/verbose events).

Guideline: **Use notes for narrative**, not for per-row detail. Use ADE events
(`events.ndjson`) for high-volume events.
...
## 9. Relationship to ADE events and run summaries

The engine also writes **ADE events** to `events.ndjson` using the unified
`ade.event/v1` envelope (see `07-telemetry-events.md` and
`11-ade-event-model.md`):

* ADE events = **timeline** (run.started, run.pipeline.progress,
  run.table.summary, run.completed, logs, errors, etc.).
...
* attaches the summary into `run.completed` events and persists it into the
  `runs` table for BI.
...
When evolving the artifact schema, keep ADE events (`ade.event/v1`) and run
summary schema (`ade.run_summary/v1`) aligned so downstream tools can continue
