...
- Set `ADE_API_LOG_LEVEL=DEBUG` (or `ADE_LOG_LEVEL=DEBUG` to apply to all services) to see debug logs; default is `INFO`. Example: `ADE_API_LOG_LEVEL=DEBUG ade dev --api-only`.
