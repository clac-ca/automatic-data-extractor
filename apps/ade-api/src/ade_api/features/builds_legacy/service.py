        ...
        *,
        ...
        if inflight_any:
            if allow_inflight and inflight_any.status is BuildStatus.QUEUED:
                logger.debug(
                    "build.prepare.claim_existing_queued",
                    extra=log_context(
                        workspace_id=spec.workspace_id,
                        configuration_id=configuration.id,
                        build_id=inflight_any.id,
                        fingerprint=inflight_any.fingerprint,
                    ),
                )
                return _BuildResolution(
                    build=inflight_any,
            ...
            if options.wait:
                logger.debug(
                    "build.prepare.wait_force_existing",
                    extra=log_context(
                        workspace_id=spec.workspace_id,
                        configuration_id=configuration.id,
                        build_id=inflight_any.id,
                ...
                # Retry resolution with wait=False to avoid endless recursion.
                return await self._resolve_build(
                    configuration=configuration,
                    spec=spec,
                    options=BuildCreateOptions(force=options.force, wait=False),
            ...
            logger.warning(
                "build.prepare.force_already_in_progress",
                extra=log_context(
                    workspace_id=spec.workspace_id,
                    configuration_id=configuration.id,
                    build_status=str(inflight_any.status),
                    build_id=inflight_any.id,
                    fingerprint=inflight_any.fingerprint,
                ),
            )
