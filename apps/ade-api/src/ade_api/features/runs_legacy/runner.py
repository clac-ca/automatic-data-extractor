...
from ade_api.schemas.event_record import EventRecord, coerce_event_record
...
__all__ = ["EngineSubprocessRunner", "RunnerFrame", "StdoutFrame", "stream_engine_events"]
async def stream_engine_events(
    frames: AsyncIterator[StdoutFrame],
) -> AsyncIterator[EventRecord]:
    """Yield NDJSON EventRecord objects from stderr frames."""

    async for frame in frames:
        if frame.stream != "stderr":
            continue
        event = coerce_event_record(frame.message)
        if event:
            yield event
