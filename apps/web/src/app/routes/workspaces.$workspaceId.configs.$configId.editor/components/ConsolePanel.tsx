...
export function ConsolePanel({
  ...
  return (
    <div className="relative flex flex-col rounded-t-2xl border border-slate-800/40 bg-slate-950/90" style={{ height }}>
      ...
      <header className="flex items-center justify-between border-b border-slate-800/60 px-4 py-2 text-xs text-slate-300">
        <div className="flex items-center gap-3">
          <button
            type="button"
            onClick={() => setTab("logs")}
            className={`rounded-full px-3 py-1 font-medium ${tab === "logs" ? "bg-brand-500/20 text-white" : "hover:bg-slate-800/60"}`}
          >
            Logs
          </button>
          <button
          ...
          >
            Problems
            {hasProblems ? <span className="ml-2 rounded-full bg-amber-500/20 px-2 text-[10px] text-amber-300">{problems.length}</span> : null}
          </button>
        </div>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-4 py-3 text-sm" aria-live="polite">
        {tab === "logs" ? (
          ...
          <ul className="space-y-2 text-[13px] text-slate-200">
            ...
            {problems.map((problem) => (
              <li key={`${problem.path}:${problem.line ?? ""}:${problem.message}`} className="rounded-xl border border-amber-500/20 bg-amber-500/10 p-3">
                <button
                  type="button"
