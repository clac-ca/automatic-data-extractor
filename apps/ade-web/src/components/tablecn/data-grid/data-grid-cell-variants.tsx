...
import { useNotifications } from "@components/providers/notifications/useNotifications";
...
import { Checkbox } from "@components/tablecn/ui/checkbox";
import {
  Command,
  CommandEmpty,
  CommandGroup,
  ...
  CommandList,
  CommandSeparator,
} from "@components/tablecn/ui/command";
import {
  Popover,
  PopoverAnchor,
  PopoverContent,
} from "@components/tablecn/ui/popover";
import {
  Select,
  SelectContent,
  SelectItem,
  SelectTrigger,
  SelectValue,
} from "@components/tablecn/ui/select";
import { Skeleton } from "@components/tablecn/ui/skeleton";
import { Textarea } from "@components/tablecn/ui/textarea";
...
import {
  ...
  getUrlHref,
  parseLocalDate,
} from "@components/tablecn/lib/data-grid";
import { cn } from "@components/tablecn/lib/utils";
import type { DataGridCellProps, FileCellData } from "@components/tablecn/types/data-grid";
...
export function FileCell<TData>({
  ...
  const addFiles = React.useCallback(
    async (newFiles: File[], skipUpload = false) => {
      ...
      if (maxFiles && files.length + newFiles.length > maxFiles) {
        ...
        notifyToast({ title: errorMessage, intent: "danger" });
        setTimeout(() => {
          setError(null);
        }, 2000);
        return;
  ...
  const removeFile = React.useCallback(
    ...
    [files, tableMeta, rowIndex, columnId, readOnly, isPending, notifyToast],
  );
  ...
  const clearAll = React.useCallback(async () => {
    ...
    if (tableMeta?.onFilesDelete && files.length > 0) {
      try {
        ...
        notifyToast({
          title: error instanceof Error ? error.message : "Failed to delete files",
          intent: "danger",
        });
