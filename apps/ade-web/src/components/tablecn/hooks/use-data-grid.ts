...
import { useAsRef } from "@components/tablecn/hooks/use-as-ref";
import { useIsomorphicLayoutEffect } from "@components/tablecn/hooks/use-isomorphic-layout-effect";
import { useLazyRef } from "@components/tablecn/hooks/use-lazy-ref";
import {
  getCellKey,
  getIsFileCellData,
  getIsInPopover,
...
function useDataGrid<TData>({
  ...
  const notifySuccess = React.useCallback(
    (title: string, description?: string) => {
      notifyToast({ title, description, intent: "success" });
    },
    [notifyToast],
  );
  const notifyError = React.useCallback(
    (title: string, description?: string) => {
      notifyToast({ title, description, intent: "danger" });
    },
    [notifyToast],
  );
  ...
  }, [store, propsRef, notifySuccess, notifyError]);
