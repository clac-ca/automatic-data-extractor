...
interface DataGridRowProps<TData> extends React.ComponentProps<"div"> {
  ...
  virtualColumnRange?: { startIndex: number; endIndex: number } | null;
  virtualColumnPaddingLeft?: number;
  virtualColumnPaddingRight?: number;
}
...
export const DataGridRow = React.memo(DataGridRowImpl, (prev, next) => {
  ...
  if (prev.shouldVirtualizeColumns !== next.shouldVirtualizeColumns) {
    return false;
  }
  if (prev.leftPinnedCount !== next.leftPinnedCount) {
    return false;
  }
  if (prev.rightPinnedCount !== next.rightPinnedCount) {
    return false;
  }
  if (
    prev.virtualColumnRange?.startIndex !== next.virtualColumnRange?.startIndex ||
    prev.virtualColumnRange?.endIndex !== next.virtualColumnRange?.endIndex
  ) {
    return false;
  }
  if (prev.virtualColumnPaddingLeft !== next.virtualColumnPaddingLeft) {
    return false;
  }
  if (prev.virtualColumnPaddingRight !== next.virtualColumnPaddingRight) {
    return false;
  }
...
function DataGridRowImpl<TData>({
  ...
  rightPinnedCount = 0,
  virtualColumnRange,
  virtualColumnPaddingLeft = 0,
  virtualColumnPaddingRight = 0,
  className,
  style,
  ref,
  ...props
  ...
  const renderCell = React.useCallback(
    (cell: (typeof visibleCells)[number], cellIndex: number) => {
      ...
      const nextCell = visibleCells[cellIndex + 1];
  ...
  return (
    ...
    >
      {shouldVirtualizeColumns && virtualColumnRange ? (
        <>
          {visibleCells.slice(0, leftPinnedCount).map((cell, idx) => {
          ...
          {virtualColumnPaddingLeft > 0 ? (
            <div
              ...
              role="presentation"
              className="shrink-0"
              style={{ width: `${virtualColumnPaddingLeft}px` }}
            />
          ) : null}

          {visibleCells
            .slice(leftPinnedCount, visibleCells.length - rightPinnedCount)
            .slice(
              virtualColumnRange.startIndex,
              virtualColumnRange.endIndex + 1,
            )
            .map((cell, idx) => {
              const cellIndex =
                leftPinnedCount + virtualColumnRange.startIndex + idx;
          ...
          {virtualColumnPaddingRight > 0 ? (
            <div
              ...
              role="presentation"
              className="shrink-0"
              style={{ width: `${virtualColumnPaddingRight}px` }}
            />
          ) : null}

          {rightPinnedCount > 0
            ? visibleCells
                .slice(visibleCells.length - rightPinnedCount)
                .map((cell, idx) => {
                  const cellIndex = visibleCells.length - rightPinnedCount + idx;
                  return renderCell(cell, cellIndex);
                })
            : null}
        </>
      ) : (
        visibleCells.map((cell, colIndex) => renderCell(cell, colIndex))
      )}
    </div>
  );
}
