...
export function DataGrid<TData>({
  ...
  const shouldVirtualizeColumns =
    enableColumnVirtualization &&
    !stretchColumns &&
    Boolean(virtualColumnRange) &&
    table.getHeaderGroups().length === 1;

  const leftPinnedCount = shouldVirtualizeColumns
    ? table.getLeftVisibleLeafColumns().length
    : 0;
  const rightPinnedCount = shouldVirtualizeColumns
    ? table.getRightVisibleLeafColumns().length
    : 0;
            ...
            >
              {shouldVirtualizeColumns ? (
                <>
                  ...
                  {headerGroup.headers
                    .slice(
                      leftPinnedCount + (virtualColumnRange?.startIndex ?? 0),
                      leftPinnedCount + (virtualColumnRange?.endIndex ?? 0) + 1,
                    )
                    .map((header, idx) => {
                      const colIndex =
                        leftPinnedCount +
                        (virtualColumnRange?.startIndex ?? 0) +
                        idx;
                  ...
                  {rightPinnedCount > 0
                    ? headerGroup.headers
                        .slice(headerGroup.headers.length - rightPinnedCount)
                        .map((header, idx) => {
                          const colIndex =
                ...
                headerGroup.headers.map((header, colIndex) => {
                  ...
                  return (
                    <div
                      ...
                      data-slot="grid-header-cell"
                      tabIndex={-1}
                      className={cn("relative", !stretchColumns && "shrink-0", {
          ...
          {virtualItems.map((virtualItem) => {
            ...
            return (
              <DataGridRow
                ...
                virtualColumnRange={virtualColumnRange}
                virtualColumnPaddingLeft={virtualColumnPaddingLeft}
                virtualColumnPaddingRight={virtualColumnPaddingRight}
              />
            );
          })}
        </div>
