...
function useDataGrid<TData>({
  ...
  const enableColumnVirtualization =
    propsRef.current.enableColumnVirtualization ?? false;
  const columnOverscan = propsRef.current.columnOverscan ?? COLUMN_OVERSCAN;

  const centerLeafColumns = React.useMemo(() => {
    if (!enableColumnVirtualization) return [];
    return table.getVisibleLeafColumns().filter((column) => !column.getIsPinned());
  }, [enableColumnVirtualization, table.getState().columnVisibility, table.getState().columnPinning, table.getState().columnOrder]);

  const columnVirtualizer = useVirtualizer({
    horizontal: true,
    count: centerLeafColumns.length,
    getScrollElement: () => dataGridRef.current,
    estimateSize: (index) => centerLeafColumns[index]?.getSize() ?? MIN_COLUMN_SIZE,
  ...
  const virtualColumnTotalSize = columnVirtualizer.getTotalSize();
  const virtualColumnItems = columnVirtualizer.getVirtualItems();

  const virtualColumnRange = React.useMemo(() => {
    if (!enableColumnVirtualization || virtualColumnItems.length === 0) return null;
    const startIndex = virtualColumnItems[0]?.index ?? 0;
    const last = virtualColumnItems[virtualColumnItems.length - 1];
    const endIndex = last?.index ?? startIndex;
    return { startIndex, endIndex };
  }, [enableColumnVirtualization, virtualColumnItems]);

  const virtualColumnPaddingLeft = React.useMemo(() => {
    if (!enableColumnVirtualization || virtualColumnItems.length === 0) return 0;
    return virtualColumnItems[0]?.start ?? 0;
  }, [enableColumnVirtualization, virtualColumnItems]);

  const virtualColumnPaddingRight = React.useMemo(() => {
    if (!enableColumnVirtualization || virtualColumnItems.length === 0) return 0;
    const last = virtualColumnItems[virtualColumnItems.length - 1];
    const end = (last?.start ?? 0) + (last?.size ?? 0);
    return Math.max(0, virtualColumnTotalSize - end);
  }, [enableColumnVirtualization, virtualColumnItems, virtualColumnTotalSize]);
  ...
  return React.useMemo(
    () => ({
      ...
      virtualColumnRange,
      virtualColumnPaddingLeft,
      virtualColumnPaddingRight,
    }),
    [
      propsRef,
      dir,
