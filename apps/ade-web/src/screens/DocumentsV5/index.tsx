...
export default function DocumentsV5Screen() {
  return (
    <RequireSession>
      <DocumentsV5Redirect />
    </RequireSession>
...
function DocumentsV5Redirect() {
  ...
  useEffect(() => {
    ...
    const target = `/workspaces/${targetWorkspace.id}/documents-v5${location.search}${location.hash}`;
  ...
  if (workspacesQuery.isLoading) {
    return <PageState title="Loading Documents v5" variant="loading" />;
  }
