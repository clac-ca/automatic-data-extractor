...
export default function DocumentsV4Screen() {
  return (
    <RequireSession>
      <DocumentsV4Redirect />
    </RequireSession>
...
function DocumentsV4Redirect() {
  ...
  useEffect(() => {
    ...
    const target = `/workspaces/${targetWorkspace.id}/documents-v4${location.search}${location.hash}`;
  ...
  if (workspacesQuery.isLoading) {
    return <PageState title="Loading Documents v4" variant="loading" />;
  }
