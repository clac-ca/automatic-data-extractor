...
describe("api key client", () => {
  ...
  it("lists my API keys with pagination flags", async () => {
    ...
    expect(client.GET).toHaveBeenCalledWith("/api/v1/users/me/api-keys", {
      params: {
  ...
  it("creates and revokes my API keys", async () => {
    ...
    expect(client.POST).toHaveBeenCalledWith("/api/v1/users/me/api-keys", {
      body: { name: "Automation", expires_in_days: 30 },
    });

    await revokeMyApiKey("key-1");
    expect(client.DELETE).toHaveBeenCalledWith("/api/v1/users/me/api-keys/{api_key_id}", {
