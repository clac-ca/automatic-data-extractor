...
import type { ConfigurationManifest, ParsedManifest, ManifestColumn, ManifestTableSection } from "./types";
...
export function parseManifest(raw: ConfigurationManifest | null | undefined): ParsedManifest {
  if (!raw) {
    return {
      name: "",
      filesHash: "",
...
export function composeManifestPatch(current: ParsedManifest, nextColumns: ManifestColumn[]): ConfigurationManifest {
  return {
    ...current.raw,
    name: current.name,
