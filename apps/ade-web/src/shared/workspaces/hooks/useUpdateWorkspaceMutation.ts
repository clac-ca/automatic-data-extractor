...
import type { WorkspaceListPage, WorkspaceProfile, WorkspaceUpdatePayload } from "../types";
  ...
  return useMutation<WorkspaceProfile, Error, WorkspaceUpdatePayload>({
    ...
    onSuccess: (workspace) => {
      ...
      queryClient.getQueriesData<WorkspaceListPage>({ queryKey: workspacesKeys.all() }).forEach(([key, page]) => {
        ...
        const items = page.items.map((entry) => (entry.id === workspace.id ? workspace : entry));
