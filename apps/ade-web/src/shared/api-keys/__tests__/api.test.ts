...
describe("api key client", () => {
  ...
  it("lists my API keys with pagination flags", async () => {
    ...
    expect(client.GET).toHaveBeenCalledWith("/api/v1/me/apiKeys", {
      params: {
  ...
  it("creates and revokes my API keys", async () => {
    ...
    expect(client.POST).toHaveBeenCalledWith("/api/v1/me/apiKeys", { body: { scope_type: "global" } });

    await revokeMyApiKey("key-1");
    expect(client.DELETE).toHaveBeenCalledWith("/api/v1/me/apiKeys/{api_key_id}", {
  ...
  it("supports admin list/get/create/revoke paths", async () => {
    ...
    expect(client.GET).toHaveBeenCalledWith("/api/v1/apiKeys/{api_key_id}", {
    ...
    expect(client.POST).toHaveBeenCalledWith("/api/v1/apiKeys", {
      body: { email: "user@example.com", scope_type: "global" },
    });

    await revokeApiKey("key-3");
    expect(client.DELETE).toHaveBeenCalledWith("/api/v1/apiKeys/{api_key_id}", {
