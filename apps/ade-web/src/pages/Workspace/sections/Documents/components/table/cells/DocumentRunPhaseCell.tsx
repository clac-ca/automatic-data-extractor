...
import type { DocumentRunPhase } from "../../../types";

const PHASE_BADGE_STYLES: Record<DocumentRunPhase, string> = {
  queued: "border-border/60 bg-secondary text-secondary-foreground",
  building: "border-border/60 bg-muted text-muted-foreground",
...
export function DocumentRunPhaseCell({
  ...
  phase: DocumentRunPhase | null;
  uploadProgress?: number | null;
}) {
  const showUpload = typeof uploadProgress === "number";
  const label = showUpload ? "Uploading" : phase ? phase.replace(/_/g, " ") : "No runs";
  const tone = phase ? PHASE_BADGE_STYLES[phase] : "border-border bg-muted text-muted-foreground";
