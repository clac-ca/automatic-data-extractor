...
- **Use the canonical homes.** Build routes via helpers in `@app/navigation`. Keep query parameter names in sync with `docs/03`, `docs/06`, `docs/07` and the filter helpers they describe (`parseDocumentFilters`, `parseRunFilters`, `build*SearchParams`).
- **Respect the layers.** Never import from an "upwards" layer (`api/`, `hooks/`, `utils/`, `components/` must not import `pages/` or `app/`). ESLint will fail fast if you try.
