...
- **Use the canonical homes.** Build routes via helpers in `@app/nav`. Keep query parameter names in sync with `docs/03`, `docs/06`, `docs/07` and the filter helpers they describe (`parseDocumentFilters`, `parseRunFilters`, `build*SearchParams`).
