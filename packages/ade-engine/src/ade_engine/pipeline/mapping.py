...
def map_columns(
    ...
    *,
    ...
    for idx, header in enumerate(headers):
        ...
        if chosen_field is None and meta:
            fallback = match_header(order, meta, normalized_header, used_fields)
...
def match_header(
    ...
    for field in order:
        ...
        if not info.get("enabled", True):
            continue
        label = normalize_header(info.get("label", field))
        synonyms = [normalize_header(value) for value in info.get("synonyms", [])]
