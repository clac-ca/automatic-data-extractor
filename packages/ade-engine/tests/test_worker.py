...
def test_run_job_normalizes_csv(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    events = list(result.events_path.read_text(encoding="utf-8").splitlines())
    assert any("job_started" in line for line in events)
    assert any("job_completed" in line for line in events)
    assert any("validation_issue" in line for line in events)
