...
- **Top bar** – Hosts workspace selector, user menu, inline system notices, and unsaved-change indicator when the current screen has dirty state. No command palette in v1.
- **Content frame** – Page header (title, status chip, primary actions) followed by a body region organized with CSS grid. Default max width 1440 px with 32 px gutters; the grid snaps to the 8 px baseline scale to keep spacing predictable.
- **Feedback** – Toast stack anchored to the top-right corner of the content frame. Long-running actions surface inline progress rows aligned with the element that triggered them, never blocking the page.
...
- Primitives rely on CSS variables exported from `frontend/src/styles/tokens.ts`. Tokens cover color, spacing, radius, typography, elevation, and focus rings. Dark mode can follow later if we choose accessible color pairs now.
- Page-level CSS lives alongside the route component using CSS modules to avoid selector collisions while keeping styling explicit.
...
- **Core interactions** – Row click routes to detail view. Inline actions restricted to “Open active configuration” and “Create new draft” so we avoid accidental promotions here.
- **Critical data contract** – `GET /document-types` returns: document type metadata, counts of draft/published configurations, last run summary (status, finishedAt), and boolean `hasAttentionItems` for the badge.
- **Technical notes** – Table uses TanStack Table with server-driven pagination and deterministic empty/error states captured in Storybook. Badges and filter chips reuse Radix primitives so we inherit accessibility without building custom roles.
...
- **Critical data contract** – `GET /document-types/:id` must deliver the document type metadata, ordered configurations with promotion eligibility flags, aggregated readiness score, outstanding issues count, and last three runs.
- **Technical notes** – Promotion flow uses Radix `AlertDialog` with explicit copy describing downstream impact. Optimistic updates stay disabled; we refresh the configuration list after the mutation resolves to keep ordering rules intact.

### 5.3 Configuration workspace
- **Purpose** – Allow configuration engineers to edit column logic, validate behavior, and see column health in one surface.
- **Layout** – Three-column grid on desktop, stacking into two sections on smaller widths:
  ...
  3. **Inspector rail** – Metadata (description, field type, optional/required toggle, sample values). History list is collapsed by default and loads lazily.
- **Core interactions** – Autosave script edits with debounce, execute column tests, publish configuration, inspect recent edit history.
- **Critical data contract** – `GET /configurations/:id` returns column list with readiness indicators, script bodies, validation errors, and metadata. `POST /configurations/:id/columns/:columnId/test` returns structured logs, sample output, and pass/fail boolean.
- **Technical notes** – Autosave uses React Query mutations with cancel + retry support and surfaces errors inline without clearing local edits. Monaco is integrated via `@monaco-editor/react` with a lightweight wrapper exposing language mode, formatting, and `onRunTest` callback.

### 5.4 Upload & run console
- **Purpose** – Provide a simple path to submit documents to a configuration and understand execution progress at a glance.
...
- **Critical data contract** – `POST /runs` accepts metadata and file handles, responding with run ID. `GET /runs/:id` returns summary status, timeline events, and per-document outcomes. `/runs/:id/stream` (SSE) emits incremental updates keyed by document ID.
- **Technical notes** – Use `react-dropzone` to manage drag-and-drop interactions with accessible fallbacks. Uploads rely on multi-part `FormData` via native `fetch`; chunking and resumable uploads stay out of scope for v1. Progress timeline entries use Radix `Accordion` for optional log expansion, and SSE events hydrate React Query caches with derived status for each file.
...
- **Critical data contract** – `GET /runs/:id/results` supports pagination, column metadata, validation flags, and CSV export URL. `GET /runs/:id/diff` returns baseline run metadata plus column-level diffs and severity.
- **Technical notes** – Favor server pagination over virtualization for v1 to keep the implementation predictable; each page loads via React Query with caching keyed by run ID + page. Diff tab reuses tokenized semantic colors for additions/removals and surfaces aggregate counts at the top. “Mark as reviewed” triggers a mutation that invalidates run list and document type library caches.

---

## 6. Interaction patterns & component layering

- **Component layering** – Domain screens live under `frontend/src/features/<domain>/pages`. Shared widgets sit in `frontend/src/components/`. Hooks that encapsulate data fetching or mutations land in `frontend/src/features/<domain>/hooks`. This keeps presentation and data concerns testable in isolation.
- **Forms** – React Hook Form + Zod drive validation. Inputs surface inline errors on blur or submit. Submit buttons stay disabled while pending and re-enable on error with retry guidance.
- **Tables & lists** – TanStack Table handles sorting/filtering. A shared `DataTable` wrapper owns skeleton, empty, and error states so every table has consistent affordances.
...
- **Keyboard & accessibility** – Provide shortcuts for editor tab switching and running tests. All focus states use tokenized outlines; ensure components expose `aria-live` updates for long-running tasks (uploads, tests).
...
| msw (Mock Service Worker) | ✅ Adopt | Powers Storybook states and component tests with predictable API mocks. |
...
- **Local persistence** – Store only display preferences (column visibility, collapsed sections) in `localStorage` via typed helpers; all business data stays server-backed.
...
2. **Fixtures** – Anonymized JSON fixtures cover empty, loading, and failure states. Storybook stories rely on Mock Service Worker (MSW) handlers to exercise edge cases without manual setup.
...
3. **Configuration workspace**
   - Ship column list, Monaco editor tabs, inspector rail, autosave, and inline testing workflow with API responses mocked via MSW.
   - Add publish/promote flows with confirmation dialogs and history drawer stub (data loads when expanded).
