...
0.1 Research review
  - [ ] Read `workpackages/wp-document-stream-refactor/research.md` lines 843-899 (perf, failure modes, SSE/WS, NOTIFY limits)
  - [ ] Read `workpackages/wp-document-stream-refactor/research.md` lines 903-929 (testing + load guidance)
  - [ ] Read `workpackages/wp-document-stream-refactor/research.md` lines 933-947 (observability metrics + logs)
1.0 Metrics and logging
  ...
  1.2 Logs
    - [ ] Log 410 resync events (Research: lines 945-946)
    - [ ] Log when SSE clients are dropped for slow consumption (Research: lines 946-947, 881-882)
2.0 Backend tests
  2.1 Unit tests
    - [ ] Token encode/decode roundtrip (Research: lines 905-912)
    - [ ] Delta ordering and 410 behavior (Research: lines 907-913)
  2.2 Integration tests
    - [ ] SSE notify -> delta -> list(id filter) flow (Research: lines 918-921)
3.0 Operational guidance
  3.1 Docs
    - [ ] Document retention maintenance steps (Research: lines 347-367, 859-863)
    - [ ] Document SSE connection limits and HTTP/2 guidance (Research: lines 867-874)
    - [ ] Document NOTIFY payload size constraints and keepalive strategy (Research: lines 845-857, 576-633)
  3.2 Load guidance
    - [ ] Capture load test guidance for bursty runs (Research: lines 923-929)

### Open Questions

- Which metrics system are we using for counters and histograms? (Research: lines 937-941)
- Do we want a lightweight load test scenario documented now or later? (Research: lines 923-929)
