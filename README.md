...
4. Poll `GET /jobs/{job_id}` (or list with `GET /jobs`, optionally filtering by `input_document_id`) to review progress, download output artefacts, and inspect metrics. Use `GET /jobs/{job_id}/tables` or `GET /documents/{document_id}/tables` to inspect extracted tables without scanning the full job list.
