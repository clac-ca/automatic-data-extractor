...
3. Start the application server (add `--reload` while you're iterating locally):
   ...
   `ade start` mirrors `uvicorn ade.app:create_app --factory`, so each invocation performs a full bootstrap before `uvicorn` comes online:
...
The FastAPI application is created in [`ade/app.py`](ade/app.py). Configuration is loaded once at startup using the `Settings` model defined in [`ade/platform/config.py`](ade/platform/config.py) and cached on `app.state.settings`.
...
Run MyPy before submitting backend changes to ensure the `ade/` package stays on a clean baseline:
...
ADE follows a feature-first layout inside the `ade/` package:

- [`ade/api`](ade/api) – minimal API shell (error handlers, settings dependency).
- [`ade/v1`](ade/v1) – top-level API router that composes feature routers.
- [`ade/features/auth`](ade/features/auth) – password, SSO, and API key flows plus access-control helpers.
- [`ade/features/documents`](ade/features/documents) – multipart uploads, metadata, downloads, and deletions.
- [`ade/features/jobs`](ade/features/jobs) – submission, status tracking, and background execution via the in-process task queue.
- [`ade/features/workspaces`](ade/features/workspaces) – routing helpers and dependencies that enforce workspace-scoped URLs.
- [`ade/features/configurations`](ade/features/configurations) – feature flags and per-workspace configuration records.
