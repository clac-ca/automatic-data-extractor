...
The FastAPI application is created in [`ade/main.py`](ade/main.py). Configuration is loaded once at startup using the `Settings` model defined in [`ade/settings.py`](ade/settings.py) and cached on `app.state.settings`.
...
- [`ade/api`](ade/api) – the versioned router shell and shared dependency aliases consumed by FastAPI routes.
